    ...
    _.each(fields, function (field) {
      ...
      if (field.field === schema.idField) {
        ...
        if (primaryKeyType instanceof DataTypes.INTEGER) {
          ...
          condition = opts.sequelize.where(lowerIfNecessary(opts.sequelize.col(aliasName + '.' + columnName)), ' LIKE ', lowerIfNecessary('%' + params.search + '%'));
        ...
        if (model.attributes[field.field] && model.attributes[field.field].type instanceof DataTypes.UUID) {
        ...
        if (!fieldNamesRequested || fieldNamesRequested.indexOf(association.as) !== -1) {
          if (['HasOne', 'BelongsTo'].indexOf(association.associationType) > -1) {
            ...
            _.each(fieldsAssociation, function (field) {
              ...
              if (hasSearchFields && !_.includes(searchAssociationFields, association.as + '.' + field.field)) {
                return;
              }
              ...
              if (field.field === schemaAssociation.idField) {
                ...
                if (modelAssociation.attributes[field.field] && modelAssociation.attributes[field.field].type instanceof DataTypes.UUID) {
                  if (params.search.match(REGEX_UUID)) {
                    condition = opts.sequelize.where(column, '=', params.search);
                    hasExtendedConditions = true;
                  }
                } else {
                  condition = opts.sequelize.where(lowerIfNecessary(column), ' LIKE ', lowerIfNecessary('%' + params.search + '%'));
                  hasExtendedConditions = true;
                }
              }
              or.push(condition);
