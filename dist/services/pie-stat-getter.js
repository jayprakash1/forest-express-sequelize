...
function PieStatGetter(model, params, opts) {
  ...
  var associationSplit, associationCollection, associationField, associationSchema, field;
  ...
  function getGroupBy() {
    return Database.isMSSQL(opts) ? [opts.sequelize.col(groupByField)] : [ALIAS_GROUP_BY];
  }
  ...
  function formatResults(records) {
    return P.map(records, function (record) {
      ...
      if (field.type === 'Date') {
        ...
        var dateonly = moment.utc(record[ALIAS_GROUP_BY]).add(offsetServer, 'h');
  ...
  this.perform = function () {
    return model.unscoped().findAll({
      attributes: [[opts.sequelize.col(groupByField), ALIAS_GROUP_BY], [opts.sequelize.fn(getAggregate(), opts.sequelize.col(getAggregateField())), ALIAS_AGGREGATE]],
      include: getIncludes(),
