...
function LineStatGetter(model, params, opts) {
  ...
  function getGroupByDateFieldFormatedForMySQL(timeRange) {
    switch (timeRange) {
      case 'day':
        return opts.sequelize.fn('DATE_FORMAT', opts.sequelize.col(groupByDateField), '%Y-%m-%d 00:00:00');
      case 'week':
        var groupByDateFieldFormated = '`' + groupByDateField.replace('.', '`.`') + '`';
        return opts.sequelize.literal('DATE_FORMAT(DATE_SUB(' + groupByDateFieldFormated + ', INTERVAL ((7 + WEEKDAY(' + groupByDateFieldFormated + ')) % 7) DAY), ' + '\'%Y-%m-%d 00:00:00\')');
      case 'month':
        return opts.sequelize.fn('DATE_FORMAT', opts.sequelize.col(groupByDateField), '%Y-%m-01 00:00:00');
      case 'year':
        return opts.sequelize.fn('DATE_FORMAT', opts.sequelize.col(groupByDateField), '%Y-01-01 00:00:00');
    }
  }
  ...
  function getGroupByDateFieldFormatedForMSSQL(timeRange) {
    switch (timeRange) {
      case 'day':
        return opts.sequelize.fn('FORMAT', opts.sequelize.col(groupByDateField), 'yyyy-MM-dd 00:00:00');
      case 'week':
        var groupByDateFieldFormated = '[' + groupByDateField.replace('.', '].[') + ']';
        return opts.sequelize.literal('FORMAT(DATEADD(DAY, -DATEPART(dw,' + groupByDateFieldFormated + '),' + groupByDateFieldFormated + '), \'yyyy-MM-dd 00:00:00\')');
      case 'month':
        return opts.sequelize.fn('FORMAT', opts.sequelize.col(groupByDateField), 'yyyy-MM-01 00:00:00');
      case 'year':
        return opts.sequelize.fn('FORMAT', opts.sequelize.col(groupByDateField), 'yyyy-01-01 00:00:00');
    }
  }
  ...
  function getGroupByDateFieldFormatedForSQLite(timeRange) {
    switch (timeRange) {
      case 'day':
        {
          return opts.sequelize.fn('STRFTIME', '%Y-%m-%d', opts.sequelize.col(groupByDateField));
        }
      case 'week':
        {
          return opts.sequelize.fn('STRFTIME', '%Y-%W', opts.sequelize.col(groupByDateField));
        }
      case 'month':
        {
          return opts.sequelize.fn('STRFTIME', '%Y-%m-01', opts.sequelize.col(groupByDateField));
        }
      case 'year':
        {
          return opts.sequelize.fn('STRFTIME', '%Y-01-01', opts.sequelize.col(groupByDateField));
        }
    }
  }
  ...
  function getGroupByDateInterval() {
    if (Database.isMySQL(opts)) {
      ...
      return [opts.sequelize.fn('to_char', opts.sequelize.fn('date_trunc', params['time_range'], opts.sequelize.literal('"' + getGroupByDateField().replace('.', '"."') + '" at time zone \'' + params.timezone + '\'')), 'YYYY-MM-DD 00:00:00'), 'date'];
    }
  }
  ...
  function getAggregate() {
    return [opts.sequelize.fn(params.aggregate.toLowerCase(), opts.sequelize.col(getAggregateField())), 'value'];
  }
